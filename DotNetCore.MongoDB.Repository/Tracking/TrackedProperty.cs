...
namespace DotNetCore.MongoDB.Repository.Tracking
{
    ...
    public interface ITrackedProperty<TValue, TVersion> : IEnumerable<PropertyRevision<TValue, TVersion>>,
        ITrackedProperty
    ...
    public class TrackedProperty<TValue, TVersion> : List<PropertyRevision<TValue, TVersion>>,
        ITrackedProperty<TValue, TVersion>
        where TVersion : IComparable
    {
        public IPropertyRevision<TValue, TVersion> GetRevision(TVersion version = default(TVersion))
        {
            return version.Equals(default(TVersion))
                ? this.LastOrDefault()
                : this.LastOrDefault(x => x.Version.CompareTo(version) <= 0);
        }
        ...
        public virtual void AddRevision(TValue value, TVersion version)
        {
            ...
            if (Count == 0)
            {
                Add(new PropertyRevision<TValue, TVersion>(value, version));
                return;
            }
        ...
        {
            return GetRevision((TVersion) version);
        }
    }
}