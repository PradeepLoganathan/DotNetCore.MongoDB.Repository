...
namespace DotNETCore.Repository.Mongo
{
        ...
        where T : IEntity
    {
        ...
        {
            return Retry(() => { return Collection.AsQueryable().Any(filter); });
        }
        ...
        {
            Collection = Database<T>.GetCollection(configuration);
        ...
        public IMongoCollection<K> GetAnotherCollection<K>() where K : IEntity
        {
        ...
        /// <param name="size">number of items in page</param>
        /// <returns>collection of entity</returns>
        public IEnumerable<T> Find(Expression<Func<T, bool>> filter, Expression<Func<T, object>> order, int pageIndex,
            int size)
        {
